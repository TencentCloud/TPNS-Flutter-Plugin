...
class _FlutterTableViewState extends State<FlutterTableView> {
  ...
  void _initScrollController() {
    ...
    this.scrollController?.addListener(() {
      double offsetY = this.scrollController?.offset ?? 0;

      if (offsetY <= 0.0) {
        this._updateCurrentSectionHeaderModel(SectionHeaderModel(), 0);
      } else {
        int section = 0;
        for (int i = 0; i < this.sectionHeaderList.length; i++) {
          SectionHeaderModel model = this.sectionHeaderList[i];
          if (offsetY >= (model?.y ?? 0) && offsetY <= (model?.sectionMaxY ?? 0)) {
            section = i;
            break;
          }
        }
        ...
        double delta = (model.sectionMaxY ?? 0) - (this.scrollController?.offset ?? 0);
        double topOffset;
        if (delta >= (model?.height ?? 0)) {
          topOffset = 0.0;
        } else {
          topOffset = delta - (model?.height ?? 0);
        }
  ...
  void _createListView() {
    ...
    this.listView = ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        Widget? itemWidget;
        RowSectionModel model = this._getRowSectionModel(index) ?? RowSectionModel(section: 0, row: 0, haveHeaderWidget: false);
        double height;
        if (model.row == 0 && (model.haveHeaderWidget ?? false)) {
          itemWidget = this.sectionHeaderList[model.section ?? 0].headerWidget;
          height = this.widget.sectionHeaderHeight(context, model.section ?? 0);
        } else {
          int row = (model.haveHeaderWidget ?? false) == false ? (model.row ?? 0) : (model.row ?? 0) - 1;
          itemWidget = this.widget.cellBuilder(context, model.section ?? 0, row);
          height = this.widget.cellHeight(context, model.section ?? 0, row);
        }
  ...
  Widget build(BuildContext context) {
    ...
    if (this.widget.listViewFatherWidgetBuilder != null) {
      listViewFatherWidget =
          this.widget.listViewFatherWidgetBuilder!(context, this.listView ?? ListView());
    }
